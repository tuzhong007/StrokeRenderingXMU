...
void main() {
    ...
    float segLength = s_ti[1] - s_ti[0];
    
    // calculate the start position of the first solid line
    float modulus = mod(s_ti[0], (l1 + l2));
    float startLength = (modulus <= l1 ? 0 : (l1 + l2) - modulus);
    ...
    for (int i = 0; i < numSolidLines; i++) {
        ...
        // calculate the end point of the solid line
        float subSegLength;
        // if some part of the solid line is rendered in the last segment, we need to subtract the length of the done part
        if (startLength < 1e-3) {
            subSegLength = min(l1 - modulus, segLength - startLength);
        }
        else {
		...
		vec2 p2_solid = quadraticBezier(p0, p1, p2, t1);
        vec2 p0p1_solid_direction = 2 * (1 - t0) * (p1 - p0) + 2 * t0 * (p2 - p1);
        vec2 p1p2_solid_direction = 2 * (1 - t1) * (p1 - p0) + 2 * t1 * (p2 - p1);
        vec2 p1_solid = getIntersection(p0_solid, p0p1_solid_direction, p2_solid, p1p2_solid_direction);
