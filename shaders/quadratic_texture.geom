...
layout(location = 7) in float s_ti[];
layout(location = 8) in float currentStartLength0[];
...
//out float currentStartLength[2]; // start length of the current solid line
...
{
    float ratio = segL / w;
    //vec3 color0 = (((segID[0]) % 2) == 0) ? vec3(0,1,1) : vec3(0,0,1);
    vec3 color0 = (((segID[0]) % 2) == 0) ? vec3(0,1,1) : vec3(1,1,0);
    ...
    p0d0 = vec4(p0, w_half * n0), p1d1 = vec4(p1, d), p2d2 = vec4(p2, w_half * n2);
    ...
    vec3 baryP0_Prime = barycentricCoordinate(p0_pp, p1_pp, p2_pp, p0 + w_half * n0);
    vec3 baryP1_Prime = barycentricCoordinate(p0_pp, p1_pp, p2_pp, p1 + d);
    vec3 baryP2_Prime = barycentricCoordinate(p0_pp, p1_pp, p2_pp, p2 + w_half * n2);
    ...
    gColor = color1;
    isConvex = 1;
    posInLocalSpace = p1_p;
    initParameters = vec2(ratio / 2, 1);
    EmitVertex();
    // p2'
    ...
    gColor = color2;
    isConvex = 1;
    posInLocalSpace = p2_p;
    initParameters = vec2(ratio, 1);
    EmitVertex();
    EndPrimitive();
    ...
    EmitVertex();
    // p0''
    gl_Position = projection * view * model * vec4(p0_pp, 0, 1);
    //gTexCoord = vec2(0.5, 0.5);
    gTexCoord = vec2(0, 0);
    if (isReversed || isDegenerate) {
        // edge case, render full color
    ...
    EmitVertex();
    // p2''
    gl_Position = projection * view * model * vec4(p2_pp, 0, 1);
    //gTexCoord = vec2(0.5, 0.5);
    gTexCoord = vec2(1, 1);
    if (isReversed || isDegenerate) {
        // edge case, render full color
    ...
    gColor = color2;
    isConvex = 0;
    posInLocalSpace = p2_pp;
    initParameters = vec2(ratio, 0);
    EmitVertex();
    // p2'
    gl_Position = projection * view * model * vec4(p2_p, 0, 1);
    //gTexCoord = vec2(0.5, 0.5);
...
void main() {
    ...
    lastTexCoord = s_ti[0] / w;
    segL = s_ti[1] - s_ti[0];
   // currentStartLength[0] = currentStartLength0[0];
    //currentStartLength[1] = currentStartLength0[1];
